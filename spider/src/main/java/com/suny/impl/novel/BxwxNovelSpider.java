    ...
    public List<Novel> getsNovel(String url) {
        ...
        try {
            ...
            for (int index = 0, size = trs.size(); index < size; index++) {
                ...
                novel.setName(tds.first().text());    // 获取标签对应文本>>>>>书名
                ...
                novel.setAuth(tds.get(2).text());   //设置作者
                // 把最后更新的时间设置进去，然后网页上拿到的是字符串对象，转为Date对象
                novel.setLastUpdateTime(NovelSpiderUtil.getDate(tds.get(3).text(), "yy-MM-dd"));   //设置更新的时间
